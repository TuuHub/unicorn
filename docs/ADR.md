...
- **Agent memory** (what unicorn believes about the user's world): **one markdown notes document** (optionally split by domain: preferences, per-course patterns) stored in D1, read in full on every reasoning call, and writable through an MCP tool so the user's own agent can persist judgments ("this course's quizzes don't count"). The resident loop's write role is **consolidation only**: near the cap it compresses the notes with a budget-capped model call (merge duplicates, drop obsolete rules); it never authors new judgments, because until the IM conversational face exists it has no correction signal to learn from. The reader is the LLM — prose is its native format, and judgments are irregular enough to resist schema.
